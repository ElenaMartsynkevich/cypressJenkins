...
describe ('Our first suite', () => {
    it('first test', () => {
        ...
        // Locator. search element by Tag Name
        cy.get('input')
    ...
    it('assert property', () =>{
        ...
        .then(input =>{
           ...
           // cy.wrap(input).invoke('prop', 'value').should('contain', 'May 17, 2023')
        })
    })
    ...
    it('web tables', () => {
        ...
        cy.get('thead').find('tr').eq(2).then (tableRow => {
            ...
            cy.wrap(tableRow).find('[placeholder="Last Name"]').type('Nikitov')
            cy.wrap(tableRow).find('.nb-checkmark').click()
        })
        cy.get('tbody tr').first().find('td').then (tableColumns =>{
            cy.wrap(tableColumns).eq(2).should('contain','Artem')
            cy.wrap(tableColumns).eq(3).should('contain','Nikitov')
        })
